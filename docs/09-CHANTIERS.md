...
> **Une migration attend un `db:push`** : `20260916120000_a_purge_prospects.sql` (16 septembre).
> Validée par `db:check`, pas appliquée sur la base hébergée — prévenir l'autre avant de la
> pousser. Elle **ajoute une fonction** : `purger_prospects_inactifs` a été déclarée à la main
> dans `packages/db/src/database.types.ts`, un `db:types:linked` après le push doit la
> retrouver à l'identique. Rien ne l'appelle tant que le planificateur n'existe pas, donc la
> pousser ne supprime personne.
