...
| Harmoniser le site au vouvoiement               | 16 sept. | Le tunnel et l'espace client tutoyaient encore (« Réserve ton audit », « Ton compte est créé »), seuls îlots du site à ne pas suivre la décision du 16 septembre. Vingt-huit fichiers : les six écrans de `/qualification`, `/reserver` et `/formations/[slug]/souscrire`, les six de `/espace`, leurs actions serveur, et surtout leur matière — `lib/qualification/questionnaire.ts` (les onze questions et titres d'écran du formulaire), `lib/messages/catalogue.ts` (les sept messages constants et dépendants qui s'affichent sur ces pages), `lib/auth/creation-compte.ts` et `lib/paiement/checkout.ts` (les erreurs remontées par les deux parcours d'achat). Les commentaires qui citaient le texte d'un message édité ont été alignés avec lui, pour ne pas laisser une citation fausse à côté du code qu'elle décrit — sauf la citation du formulaire Tally externe (`eligibilite.ts`) et les deux narrations d'un bug déjà mort avant la refonte des messages (`catalogue.ts`, `reserver/page.tsx`), qui restent ce qu'elles décrivaient. `/connexion` et le reste du site public étaient déjà au vouvoiement. Trouvé en relisant chaque fichier plutôt qu'en cherchant par pronom, qui manque les formes élidées (`ne t’appartient pas`) et les inversions (`es-tu`, `as-tu`). Typecheck, lint et Prettier passent ; aucune migration. |
