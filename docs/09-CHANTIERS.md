...
| **Voir les messages à l'écran**                          | Le système de messages du 13 septembre n'a été vu que sur des codes constants, sur `/reserver`, en HTML brut. **Trois choses n'ont jamais été affichées** : le ton `alerte` — donc le `role="alert"` du composant, dont seule la branche `status` est prouvée —, les deux rendus dépendants de `/espace/communaute` (`roleEnFile` vrai et faux), et le retour `compte-cree` en situation vraie. Tout est faisable depuis un poste qui atteint la base : une session cliente suffit pour le ton `alerte` et pour `/espace/communaute`, et un passage par `/qualification` pour `compte-cree`. À regarder aussi au lecteur d'écran : c'est le seul endroit où la dérivation du rôle se constate vraiment. |
