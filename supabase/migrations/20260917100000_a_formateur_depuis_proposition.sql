-- ═══════════════════════════════════════════════════════════════════════════
-- Le formateur d'un accès payé : celui qui a émis la proposition
--
-- `traiter_paiement()` ouvrait l'inscription sans formateur, et rien d'autre ne
-- le posait : aucun formateur ne voyait donc jamais un client réel — la RLS de
-- son espace s'ancre sur `inscriptions.formateur_id`. Au lancement, un seul
-- formateur vend et suit (Franck) : une affectation à la main après chaque
-- vente serait un geste qu'on finit par oublier. Le vendeur devient donc le
-- formateur par défaut ; le back-office permet de désigner quelqu'un d'autre.
--
-- Seul changement par rapport à 20260908110000 : le bloc « proposition ».
-- ═══════════════════════════════════════════════════════════════════════════
